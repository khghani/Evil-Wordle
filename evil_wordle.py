...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        for i, letter in enumerate(guessed_word):
            ...
            elif color is WRONG_SPOT_COLOR and self.colors[letter] not in CORRECT_COLOR:
...
def fast_sort(lst):
    ...
    center_less = [x for x in lst if x < center]
    center_middle = [x for x in lst if x == center]
    center_greater = [x for x in lst if x > center]
    return fast_sort(center_less) + center_middle + fast_sort(center_greater)
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for word in remaining_secret_words:
        model = tuple(get_feedback_colors(word, guessed_word))
        if model not in word_groups:
            word_groups[model] = []
        word_groups[model].append(word)
    word_families = [WordFamily(pattern, words) for pattern, words in word_groups.items()]
    word_families = fast_sort(word_families)
    difficult_family = word_families[0]
    feedback_colors = difficult_family.feedback_colors
    remaining_secret_words = difficult_family.words
